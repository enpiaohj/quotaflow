...
以下修复均已发布，但尚未在真实条件下确认。确认一项就划掉一项，避免它们无声地留在代码里。

| 项 | 版本 | 未确认的原因 | 如何确认 |
| --- | --- | --- | --- |
| Claude 限流退避以服务端 `Retry-After` 为准 | v0.8.0 | 无法主动触发 Anthropic 的 429 | 下次出现限流时，看提示是否变成「将在 N 分钟后自动重试」，且额度数字仍在 |
| 代理修复（环境变量优先导致全平台失败） | v0.9.1 | 无法在出问题的那台机器上实测 | 在该机器把「网络代理」改为「直连」；若仍失败，用「生成诊断报告」把报告发回 |
| `per1WeekPercentage` 按比例解读（0.4696 = 46.96%） | v0.6.0 | 阿里云无公开文档 | 与百炼控制台页面显示的百分比比对 |

遇到问题时，优先用 **设置 → 数据与隐私 → 生成诊断报告**：它会输出版本、代理配置、
各平台状态与最近异常类型，且已脱敏（不含密钥、登录态与额度数值），可直接发送。
